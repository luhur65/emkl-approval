...
<div class="container-fluid">
    ...
    <div class="card card-primary card-outline">
        <div class="card-body">
            ...
            <!-- Daftar ini SELALU berisi permintaan cetak ulang yg belum di-approve
                 (FIsCetak = 1); tidak ada filter status spt di halaman approval
                 harga, karena aksinya satu arah -- lihat catatan pd tombolnya. -->
            <div class="row mt-2">
                <div class="col-12">
                    <small class="text-muted">
                        <i class="fas fa-info-circle"></i>
                        Menampilkan penawaran harga yang meminta cetak ulang pada tanggal terpilih.
                    </small>
                </div>
            </div>
        </div>
...
<script>
    var apiUrl = "<?= base_url('approvalpharga/ajax_list_cetak') ?>" + window.location.search;
    var $grid = $("#jqGrid");
    ...
    // diproses. UPDATE-nya memakai WHERE ber-syarat FIsCetak = 1 & tanggal, jadi
    // baris yg keburu di-approve user lain memang tidak berubah -- dan itu HARUS
    // terlihat, bukan ikut terhitung berhasil spt di CI3.
    function pesanHasilApproval(hasil) {
        ...
        if (dilewati.length) {
            baris.push('');
            baris.push('Dilewati ' + dilewati.length + ' data (permintaan cetak ulangnya sudah di-approve, atau bukan dari tanggal ini):');
    ...
    // multi-baris yg memuat kiriman server. Escape dulu lewat .text(), baru
    // newline-nya diubah jadi <br> supaya tetap terbaca sbg baris terpisah.
    ...
    // --- Menu APPROVE CETAK ULANG (modal customPager) ------------------------
    // Satu aksi & satu arah: FIsCetak 1 -> 0. Tidak ada pasangan "batalkan" di
    // CI3, dan tidak ditambahkan di sini -- permintaan cetak ulang dibuat dari
    // aplikasi lain, bukan dari halaman ini.
    function prosesApproval() {
        ...
        showConfirm(
            'Konfirmasi APPROVE CETAK ULANG',
            'Approve permintaan cetak ulang untuk ' + selectedRows.length + ' penawaran harga terpilih?'
        ).then(function() {
            kirimApproval();
        });
    }
    ...
    function kirimApproval() {
        ...
        $.ajax({
            type: "POST",
            url: "<?= base_url('approvalpharga/approved_cetak') ?>",
            beforeSend: function() {
            ...
            success: function(result) {
                // Baris yg berhasil diproses langsung keluar dari daftar
                // (FIsCetak-nya sudah 0), jadi seleksinya sudah tidak relevan.
                clearSelectedRows();
                reloadCetakGrid();
                showDialog({
                    statuspesan: 'success',
    ...
    // yg ada di memori grid (mis. 50 dari 1000).
    function selectAllRows() {
        var $selectAll = $('.checkbox-selectall');
        var post = $grid.jqGrid('getGridParam', 'postData') || {};

        $.ajax({
            url: '<?= base_url('approvalpharga/select_all_ids_cetak') ?>',
            type: 'GET',
            dataType: 'json',
    ...
    // Muat ulang grid dari halaman 1. Lazy loading pakai datatype:'local', jadi
    // reload HARUS lewat loadGridData() (server) -- trigger('reloadGrid') bawaan
    // jqGrid cuma render ulang data lokal.
    function reloadCetakGrid(callback) {
    ...
    // WAJIB dibuang. Sort & filter kolom sengaja TIDAK membuang seleksi:
    ...
    function reloadCetakGridKeepFocus() {
    ...
    // Tidak mengandalkan autosearch/searchOnEnter bawaan filterToolbar; `filters`
    // dibangun sendiri dgn format yg sama persis dipakai globalSearch.
    ...
    $(document).ready(function() {
        ...
        $grid.jqGrid({
            ...
            colModel: [
                {
                    ...
                    searchoptions: {
                        ...
                        dataInit: function(element) {
                            ...
                            // TIDAK memicu reloadGrid bawaannya sendiri, yg pd grid
                            // datatype:'local' cuma me-repaginate baris di memori &
                            // membuat nomor baris lazy-load jadi rancu.
            ...
            loadComplete: function() {
                ...
                // asli. Hitung posisi absolut tiap baris dari cache lazy-load.
        ...
        // rownumbers:true bawaan jqGrid. Left/Right tidak dipasang: tanpa subgrid.
        ...
        // triggerColumnFilterSearch(). Pencarian global memakai `filters` berformat
        // sama; keduanya diproses di GridPipeline::filterRows().
        ...
        // menandainya .is-visible saat input kolom ybs terisi.
        ...
        $grid.globalSearch({
            ...
            reload: reloadCetakGridKeepFocus
        }).customPager({
            lazyLoading: true,
            modalBtnList: [{
                id: 'approve',
                title: 'Approve',
                caption: 'Approve',
                innerHTML: '<i class="fa fa-print"></i> CETAK ULANG',
                class: 'btn btn-purple btn-sm mr-1 ',
                item: [{
                    id: 'btnApproveCetak',
                    color: 'btn-success',
                    text: 'Approve Cetak Ulang',
                    onClick: function() { prosesApproval(); }
                }]
            }]
        });
