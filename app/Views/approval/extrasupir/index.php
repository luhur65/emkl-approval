                ...
                <div class="col-md-3">
                    <div class="form-group filter-input-group">
                        ...
                        <div class="input-group date">
                            ...
                            <input type="text" class="form-control datepicker" id="tgldari" autocomplete="off" value="<?= date('d-m-Y') ?>">
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group filter-input-group">
                        ...
                        <div class="input-group date">
                            ...
                            <input type="text" class="form-control datepicker" id="tglsampai" autocomplete="off" value="<?= date('d-m-Y') ?>">
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group filter-input-group">
                        ...
                        <!-- Nilainya menentukan isi grid SEKALIGUS aksi yang masuk akal
                             untuk isi itu: 0 = baris yang belum di-approve (tombol
                             APPROVE), 1 = yang sudah (tombol BATAL APPROVE). CI3
                             memakai FALSE/TRUE dengan arti yang persis sama, dan
                             default-nya juga daftar "Un Approved". -->
...
<script>
    var apiUrl = "<?= base_url('approvalextrasupir/ajax_list') ?>" + window.location.search;
    var $grid = $("#jqGrid");
    ...
    // Datepicker menampilkan d-m-Y; server & SP memakai Y-m-d.
    function keTanggalServer(nilai) {
        var d = (nilai || '').split('-');
        return d.length === 3 ? d[2] + '-' + d[1] + '-' + d[0] : '';
    }

    function tglDari() {
        return keTanggalServer($('#tgldari').val());
    }

    function tglSampai() {
        return keTanggalServer($('#tglsampai').val());
    }
    ...
    function pesanHasilApproval(hasil) {
        ...
        if (dilewati.length) {
            baris.push('');
            baris.push('Dilewati ' + dilewati.length + ' data (sudah diproses orang lain, atau bukan dari rentang tanggal ini):');
    ...
    // (dikirim sbg `proses`) DAN aksi mana yg masuk akal untuk baris itu. Jadi
    ...
    var AKSI_APPROVE = {
        prosesdata: '0',
        label: 'APPROVE',
        url: "<?= base_url('approvalextrasupir/approved') ?>",
        tanya: function(jumlah) {
            return 'Approve biaya extra supir untuk ' + jumlah + ' data terpilih?';
        }
    };
    ...
    var AKSI_UNAPPROVE = {
        prosesdata: '1',
        label: 'Un Approve',
        url: "<?= base_url('approvalextrasupir/unapproved') ?>",
        tanya: function(jumlah) {
            return 'Batalkan approve biaya extra supir untuk ' + jumlah + ' data terpilih?';
        }
    };
    ...
    function selectAllRows() {
        ...
        $.ajax({
            url: '<?= base_url('approvalextrasupir/select_all_ids') ?>',
            type: 'GET',
            dataType: 'json',
            data: $.extend({}, post, {
    ...
    // Kembalikan kartu filter ke kondisi awal halaman (rentang tanggal hari ini,
    // proses data ke pilihan pertama) lalu buang pencarian global + filter
    // per-kolom, sehingga grid tampil persis spt saat halaman baru dibuka.
    function resetFilterGrid() {
        $('#tgldari').val('<?= date('d-m-Y') ?>');
        $('#tglsampai').val('<?= date('d-m-Y') ?>');
    ...
    $(document).ready(function() {
        ...
        $grid.jqGrid({
            ...
            postData: {
                tgldari: tglDari,
                tglsampai: tglSampai,
                proses: function() { return $('#prosesdata').val(); }
            },
            colModel: [
                {
                    label: 'NO.',
                    ...
                    width: 60,
                    align: 'left',
                    // Kolom ini TIDAK ikut dilebarkan shrinkToFit (yang di grid ini
                    // memang dimatikan); `fixed` adalah cara jqGrid sendiri
                    // mengecualikan kolom dari pembagian lebar container.
                    fixed: true,
                ...
                // Urutan & judul kolom mengikuti <thead> milik view CI3. Yang
                // berubah hanya PASANGAN datanya: di CI3 kolom mulai "Shipper"
                // bergeser satu terhadap judulnya, dan "No Polisi" tidak pernah
                // terisi -- lihat ApprovalExtraSupirService::mapRows().
                { label: 'Keterangan Extra', name: 'FKetByExtra', width: 220 },
                { label: 'Biaya Extra Supir', name: 'FNomSupirByExtra', width: 145, align: 'right' },
                { label: 'Tagih Ke Emkl', name: 'FNomCustByExtra', width: 140, align: 'right' },
                { label: 'No Trip', name: 'FNTrans', width: 150 },
                { label: 'Tanggal Trip', name: 'FTgl', width: 115, align: 'center' },
                { label: 'Shipper', name: 'FShipper', width: 240 },
                { label: 'Dari', name: 'FDari', width: 160 },
                { label: 'Sampai', name: 'FSampai', width: 160 },
                { label: 'Gaji Supir', name: 'FGajiSupir', width: 130, align: 'right' },
                { label: 'Jenis Orderan', name: 'FJnsOrder', width: 130 },
                { label: 'No Job', name: 'FNoJob', width: 140 },
                { label: 'No Container', name: 'FNoCont', width: 140 },
                { label: 'No Seal', name: 'FNoSeal', width: 120 },
                { label: 'Full/Empty', name: 'FStatusCont', width: 110 },
                { label: 'Gudang', name: 'FNamaGudang', width: 180 },
                { label: 'No Polisi', name: 'FKGdg', width: 120 },
                { label: 'Supir', name: 'FKSupir', width: 180 },
                { label: 'EMKL', name: 'FEMKL', width: 160 },
                { label: 'Container', name: 'FCont', width: 110 },
                { label: 'Tujuan', name: 'FTujuan', width: 160 },
                { label: 'Job Trucking', name: 'FJobTrucking', width: 140 }
            ],
            autowidth: true,
            // Dimatikan, tidak seperti modul approval lain: dengan 21 kolom data
            // shrinkToFit akan memampatkan semuanya ke lebar container sampai isi
            // kolom terpotong. Lebar per kolom dipertahankan, grid-nya yang
            // digeser mendatar.
            shrinkToFit: false,
            height: 360,
            rowNum: 50,
            ...
            loadComplete: function() {
                ...
                if (state && ids.length) {
                    ...
                    ids.forEach(function(id, i) {
                        ...
                        // jqID WAJIB: id baris di sini No Trip, yang bisa memuat
                        // karakter yang bikin jQuery menolak selector 'tr#...'
