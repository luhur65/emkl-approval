...
<script>
    var apiUrl = "<?= base_url('approvaltrip/ajax_list') ?>" + window.location.search;
    var $grid = $("#jqGrid");
    ...
    var AKSI_APPROVE = {
        prosesdata: '0',
        label: 'APPROVE',
        url: "<?= base_url('approvaltrip/approved') ?>",
    ...
    var AKSI_UNAPPROVE = {
        prosesdata: '1',
        label: 'UN APPROVE',
        url: "<?= base_url('approvaltrip/unapproved') ?>",
        tanya: function(jumlah) {
            return 'Un-approve ' + jumlah + ' data terpilih?';
        }
    };
    ...
    function selectAllRows() {
        ...
        $.ajax({
            url: '<?= base_url('approvaltrip/select_all_ids') ?>',
            type: 'GET',
            dataType: 'json',
            data: $.extend({}, post, {
    ...
    $(document).ready(function() {
        ...
        $grid.jqGrid({
            ...
            colModel: [
                ...
                { label: 'ID', name: 'FID', hidden: true },
                { label: 'Tanggal', name: 'FTgl', width: 120, align: 'center' },
                { label: 'Jumlah Trip', name: 'FJlhTrip', width: 120, align: 'right' },
                { label: 'Mandor', name: 'FMandor', width: 300 },
                { label: 'User ID', name: 'FUserID', width: 150 }
            ],
            autowidth: true,
            ...
            loadComplete: function() {
                ...
                if (state && ids.length) {
                    ...
                    ids.forEach(function(id, i) {
                        ...
                        // jqID: id baris berasal dari data (FID), jadi tanda baca/spasi
                        // di dalamnya akan dibaca jQuery sbg bagian dari selector --
                        // sama spt setCustomBindKeys() di bawah.
