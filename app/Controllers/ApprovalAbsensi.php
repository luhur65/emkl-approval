...
use App\Services\ApprovalAbsensiService;
use CodeIgniter\Database\Exceptions\DatabaseException;
use CodeIgniter\HTTP\IncomingRequest;
use CodeIgniter\HTTP\ResponseInterface;

/**
 * CATATAN migrasi: controller CI3 (ApprovalAbsensi.php) juga memuat cetak(),
 * ajax_list_cetak(), dan approved_cetak(). Ketiganya sisa salin-tempel dari
 * ApprovalPHarga -- isinya me-render view approval/pharga/cetak dan bekerja
 * pada TPenawaranHarga, sama sekali tidak berhubungan dengan absensi, dan
 * fungsinya sudah tersedia di ApprovalPharga. Sengaja tidak ikut dipindahkan.
 */
class ApprovalAbsensi extends BaseController
{
    protected $approvalAbsensiService;

    public function __construct()
    {
        $this->approvalAbsensiService = new ApprovalAbsensiService();
    }

    /**
     * Kode proses yang dipakai ApprovalAbsensiService: 0 menyisipkan penanda
     * approval, 1 menghapusnya. Nilai yang sama juga jadi parameter `bit`
     * pengisi grid, sehingga untuk baris yang sedang tampil hanya satu aksi
     * yang masuk akal.
     */
    ...
    public function index()
    {
        ...
        return $this->render('approval/absensi/index', [
            'title' => 'Approval Absensi Jam 9',
        ]);
    }
    ...
    public function ajax_list()
    {
        ...
        try {
            return $this->response->setJSON(
                $this->approvalAbsensiService->getGridList($this->gridParams())
            );
    ...
    public function select_all_ids()
    {
        ...
        try {
            return $this->response->setJSON(
                $this->approvalAbsensiService->getFilteredKeys($this->gridParams())
            );
        } catch (DatabaseException $e) {
            return $this->jsonErrorDatabase($e, self::NAMA_DB);
        }
     ...
     * menukar satu nilai form -- dan di modul ini "batal" berarti DELETE.
     */
    private function jalankanProses(string $prosesdata): ResponseInterface
    {
        ...
        try {
            $hasil = $this->approvalAbsensiService->processApproval(
    ...
    /**
     * Membaca daftar kunci yang dikirim view.
     *
     * Berbeda dari CI3, yang dikirim HANYA kunci -- bukan seluruh isi baris
     * (`$_POST['data']`). Nilai yang ditulis ke database diturunkan ulang di
     * server dari daftar yang layak; lihat ApprovalAbsensiService::processApproval().
     *
